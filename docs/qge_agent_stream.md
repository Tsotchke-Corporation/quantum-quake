...
evidence. It also audits `qge_moonlab_submission_packet.json` against those
...
its executable bit intact. A single postpack audit runner executes the
...
reproduce/postpack audit verdict. Packed asset-resource ledgers, resource
boundary ledgers, Moonlab full-game plan artifacts, Moonlab job plan artifacts,
...
frame, exact `frame_001.png` through `frame_N.png` membership, log, input,
trace binary byte count, trace-summary, performance, audio-byte, and Noesis
files before stream status evidence is accepted. The agent-stream ICC JSONL is
