...
- `tools/qge_breadth_evidence.py --min-maps` now enforces distinct-map breadth
  ...
  while the registered assets and captures are still incomplete. Its
  `target_map_status` rows separate ready maps, non-ready capture attempts, and
  ...
  `run_missing_maps.sh`; publication packs include that queue JSON, script, and
  Markdown under `resource/`, and `tools/qge_full_game_capture_queue_audit.py`
  rebuilds the queue from recorded reproduction inputs during postpack audit to
  reject stale JSON, script, or Markdown output. The queue now inventories local loose/Pak BSP
...
- `tools/qge_publication_pack.py --breadth-evidence` now carries that multi-map
  ...
  source, breadth aggregation, and registered-asset intake flag checks. Packed advantage
  ...
  are recomputed from their source artifacts before the postpack audit accepts
  them. Packed Markdown evidence reports are regenerated from their source JSON
  ledgers before they are treated as current, and Moonlab `.moonlab` circuit
  body files are hashed against their JSON records before control-plane payload
  ...
  exact `frame_001.png` through `frame_N.png` sequence, log, input, trace binary
  byte count, trace-summary, performance, audio-byte, and Noesis files before
  stream status evidence is accepted. The agent-stream ICC JSONL is rebuilt from
  that packed agent manifest. The packed trace summary is regenerated from
  `capture/qge_trace.bin` before trace-derived runtime evidence is accepted.
  The primary packed vanilla matrix is rebuilt from its recorded capture
  directory before its conformance counters are accepted. Packed breadth
  evidence is regenerated from the recorded source matrix ledgers before
  all-map coverage counters are accepted.
