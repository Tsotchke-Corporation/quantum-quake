...
- `tools/qge_publication_pack.py --breadth-evidence` now carries that multi-map
  ...
  boundary ledgers and deployment-gate JSON/ICC artifacts are recomputed from
  their source artifacts before the postpack audit accepts them. Packed
  Markdown evidence reports are regenerated from their source JSON ledgers
  before they are treated as current, and Moonlab `.moonlab` circuit body files
  are hashed against their JSON records before control-plane payload evidence is
  accepted.
