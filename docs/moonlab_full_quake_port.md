...
artifacts before the gate accepts them. The resource envelope and native
backend boundary ledgers are rebuilt from the oracle, advantage, vanilla,
...
coverage, unsafe shell fragments, exact core source flags, breadth aggregation
flags, and registered-asset intake flags; the
...
advantage metrics, asset-resource ledgers, resource boundary ledgers, Moonlab
...
before the postpack audit accepts them.
Packed Markdown evidence reports are regenerated from their source JSON ledgers
before they are treated as current.
...
packed agent-stream manifest is checked against packed frame files including
the exact `frame_001.png` through `frame_N.png` sequence, log, input, trace
binary byte count, trace-summary, performance, audio-byte, and Noesis files
...
directory before its conformance counters are accepted. Packed breadth evidence
is regenerated from the recorded source matrix ledgers before all-map coverage
counters are accepted. The remaining 23 maps are still asset unavailable.
