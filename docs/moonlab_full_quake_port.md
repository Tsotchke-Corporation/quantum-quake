...
`diagnostics/publication_pack/20260524-124933`, ICC reports
...
The current bundle is `calibration_payload_ready_oracle_transpilation_required`:
the readout payload is executable, but the full reversible `Q_f`/Grover oracle
is still abstract QGE QAE text, so a full QAE hardware submission is not
