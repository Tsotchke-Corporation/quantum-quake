...
`diagnostics/publication_pack/20260523-150118`, ICC reports
...
single-player map ledger: 8/32 maps covered, 24 maps pending, status `partial`.
...
1. Expand the ready matrix beyond the current publication capture and the
   8/32 partial full-game map coverage ledger. Generate the missing-map run
   script with
   `tools/qge_full_game_capture_queue.py diagnostics/publication_pack/20260523-150118`.
   The generated queue inventories local BSPs first; with the current
   `assets/id1/pak0.pak`, `start` is the only locally queueable missing map and
   23 registered maps require additional registered BSP assets before capture.
