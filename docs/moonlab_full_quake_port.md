...
`diagnostics/publication_pack/20260523-143208`, ICC reports
...
single-player map ledger: 7/32 maps covered, 25 maps pending, status `partial`.
...
1. Expand the ready matrix beyond the current publication capture and the
   7/32 partial full-game map coverage ledger. Generate the missing-map run
   script with
