#!/usr/bin/env python3
"""Audit oracle scene evidence against its source capture."""
...
ORACLE_SCENE_SCHEMA = "qge.scene_oracle_ir.v0"
ORACLE_SCENE_FORBIDDEN_FLAGS = (
...
def expected_oracle_scene_from_source(
    recorded_oracle_scene: dict[str, Any],
) -> dict[str, Any]:
    source = dict_or_empty(recorded_oracle_scene.get("source_capture"))
    capture_dir = path_or_none(source.get("capture_dir"))
    claims_ledger = path_or_none(source.get("claims_ledger"))
    if capture_dir is None:
        raise ValueError("oracle_scene.source_capture.capture_dir is missing")
    if claims_ledger is None:
        raise ValueError("oracle_scene.source_capture.claims_ledger is missing")
    oracle_scene, _claims_evidence = qge_oracle_export.build_oracle_scene(
        capture_dir,
        claims_ledger,
    )
    return oracle_scene
...
def oracle_scene_source_audit(
    oracle_scene: dict[str, Any] | None,
    *,
    expected_oracle_scene: dict[str, Any] | None = None,
    required: bool = False,
) -> dict[str, Any]:
    recorded = dict_or_empty(oracle_scene)
    ...
    try:
        expected = (
            dict_or_empty(expected_oracle_scene)
            if expected_oracle_scene is not None
            else expected_oracle_scene_from_source(recorded)
        )
    except (OSError, KeyError, TypeError, ValueError) as exc:
        build_errors.append({
            "artifact": "oracle_scene",
            "error": str(exc),
        })
        expected = {}
    ...
    schema_mismatch = bool(
        recorded and recorded.get("schema") != ORACLE_SCENE_SCHEMA)
    ...
    overclaim_flags = (
        qge_moonlab_overclaim_audit.recursive_overclaim_flags(
            "oracle_scene",
            recorded,
            forbidden=ORACLE_SCENE_FORBIDDEN_FLAGS,
        )
        if recorded else []
    )
    mismatch_count = (
...
def oracle_scene_source_audit_from_manifest(
    manifest: dict[str, Any],
    *,
    required: bool = True,
) -> dict[str, Any]:
    return oracle_scene_source_audit(
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fail-on-mismatch",
        action="store_true",
        help="Exit nonzero when oracle scene evidence is stale.",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        audit = oracle_scene_source_audit_from_manifest(
            load_json(resolve_manifest(args.pack_or_manifest)),
            required=True,
        )
        if args.out is not None:
            write_json(args.out, audit)
            print(f"QGE_ORACLE_SCENE_AUDIT {args.out}")
        else:
            print(json.dumps(audit, indent=2, sort_keys=True))
    except (OSError, ValueError, KeyError, TypeError) as exc:
        print(f"qge_oracle_scene_audit: {exc}", file=sys.stderr)
        return 1
