...
cvar_t quantum_render_update_interval = {"quantum_render_update_interval", "1", CVAR_ARCHIVE};
...
#define QGE_TEXTURE_BILINEAR_MIN_FOOTPRINT 1.00f
...
static int qge_noesis_assist_hidden_chase_target_id = 0;
static int qge_noesis_assist_hidden_chase_since_frame = -1;
static int qge_noesis_assist_hidden_cooldown_target_id = 0;
static int qge_noesis_assist_hidden_cooldown_until_frame = -1;
static int qge_noesis_assist_hidden_chase_timeouts_total = 0;
static qboolean qge_noesis_assist_hidden_chase_timeout = false;
static int qge_noesis_assist_hidden_wall_target_id = 0;
static int qge_noesis_assist_hidden_wall_since_frame = -1;
static int qge_noesis_assist_hidden_wall_timeouts_total = 0;
static qboolean qge_noesis_assist_hidden_wall_timeout = false;
...
static int qge_scene_world_gap_repairs = 0;
static int qge_scene_texture_prefilter_samples = 0;
...
typedef struct {
	...
	float light_s_num_dy;
	float light_t_num_dx;
	float light_t_num_dy;
	float w0_origin;
	float w1_origin;
	float w0_dx;
	float w0_dy;
...
static void QGE_GameplayOutcomeResetState(void)
{
	...
	qge_noesis_assist_hidden_chase_target_id = 0;
	qge_noesis_assist_hidden_chase_since_frame = -1;
	qge_noesis_assist_hidden_cooldown_target_id = 0;
	qge_noesis_assist_hidden_cooldown_until_frame = -1;
	qge_noesis_assist_hidden_chase_timeouts_total = 0;
	qge_noesis_assist_hidden_chase_timeout = false;
	qge_noesis_assist_hidden_wall_target_id = 0;
	qge_noesis_assist_hidden_wall_since_frame = -1;
	qge_noesis_assist_hidden_wall_timeouts_total = 0;
	qge_noesis_assist_hidden_wall_timeout = false;
...
static qboolean QGE_NoesisAssistHiddenCoolingDown(int target_id,
												 qboolean visible)
{
	if (visible || target_id <= 1 ||
		target_id != qge_noesis_assist_hidden_cooldown_target_id)
		return false;
	if (qge_frame_count > qge_noesis_assist_hidden_cooldown_until_frame) {
		qge_noesis_assist_hidden_cooldown_target_id = 0;
		qge_noesis_assist_hidden_cooldown_until_frame = -1;
		return false;
	}
...
static void QGE_NoesisAssistCooldownHiddenTarget(int target_id)
{
	qge_noesis_assist_hidden_cooldown_target_id = target_id;
	qge_noesis_assist_hidden_cooldown_until_frame =
		qge_frame_count + QGE_NOESIS_HIDDEN_CHASE_COOLDOWN_FRAMES;
	qge_noesis_assist_hidden_chase_target_id = 0;
	qge_noesis_assist_hidden_chase_since_frame = -1;
	qge_noesis_assist_locked_target_id = 0;
	qge_noesis_assist_target_lock_until_frame = -1;
	qge_noesis_assist_wall_follow_until_frame = -1;
	QGE_NoesisAssistResetHiddenWallStall();
}

static qboolean QGE_NoesisAssistHiddenWallStalled(int target_id,
												 qboolean visible,
												 float distance,
												 float forward_clear)
{
	if (visible || qge_noesis_autonomous.value < 0.5f ||
		target_id <= 1 ||
		distance < 0.0f ||
	...
	if (target_id != qge_noesis_assist_hidden_wall_target_id) {
		qge_noesis_assist_hidden_wall_target_id = target_id;
		qge_noesis_assist_hidden_wall_since_frame = qge_frame_count;
		return false;
	}
	if (qge_noesis_assist_hidden_wall_since_frame < 0) {
		qge_noesis_assist_hidden_wall_since_frame = qge_frame_count;
		return false;
	}
	return qge_frame_count -
		qge_noesis_assist_hidden_wall_since_frame >=
		QGE_NOESIS_HIDDEN_WALL_STALL_FRAMES;
}
										  ...
										  qboolean *locked_out)
{
	...
	if (allow_target_lock &&
		...
		if (locked && !locked->free && locked->v.health > 0.0f &&
			...
			if (!QGE_NoesisAssistHiddenCoolingDown(NUM_FOR_EDICT(locked),
												  visible) &&
	...
	for (int i = 2; i < sv.num_edicts; i++) {
		...
		if (!visible &&
			QGE_NoesisAssistHiddenCoolingDown(NUM_FOR_EDICT(ent), false))
			continue;
		if (distance < best_nearest_distance) {
			best_nearest_distance = distance;
			best_nearest = ent;
			if (visible) {
				VectorCopy(candidate_aim, best_nearest_aim);
			}
			else {
								 ...
								 usercmd_t *move)
{
	...
	if (!chase_mode || visible) {
		qge_noesis_assist_hidden_chase_target_id = 0;
		qge_noesis_assist_hidden_chase_since_frame = -1;
	}
	else if (qge_noesis_autonomous.value >= 0.5f) {
		if (selected_target_id != qge_noesis_assist_hidden_chase_target_id) {
			qge_noesis_assist_hidden_chase_target_id = selected_target_id;
			qge_noesis_assist_hidden_chase_since_frame = qge_frame_count;
		}
		else if (qge_noesis_assist_hidden_chase_since_frame >= 0 &&
				 qge_frame_count -
				 qge_noesis_assist_hidden_chase_since_frame >=
				 QGE_NOESIS_HIDDEN_CHASE_VISIBILITY_TIMEOUT_FRAMES) {
			qge_noesis_assist_hidden_chase_timeout = true;
			qge_noesis_assist_hidden_chase_timeouts_total++;
			QGE_NoesisAssistCooldownHiddenTarget(selected_target_id);
	...
	if (chase_mode && engage_target && !visible &&
		...
		right_clear = QGE_NoesisAssistTraceDistance(player,
													anglemod(aim[YAW] - 45.0f));
		if (QGE_NoesisAssistHiddenWallStalled(selected_target_id, visible,
											 distance, forward_clear)) {
			qge_noesis_assist_hidden_wall_timeout = true;
			qge_noesis_assist_hidden_wall_timeouts_total++;
			qge_noesis_assist_hidden_chase_timeout = true;
			qge_noesis_assist_hidden_chase_timeouts_total++;
			QGE_NoesisAssistCooldownHiddenTarget(selected_target_id);
			qge_noesis_assist_mode = chase_mode ? 2 : 1;
...
static void QGE_GameplayOutcomeSample(void)
{
	...
	fprintf(qge_gameplay_outcome_file,
		...
		"\"hidden_chase_timeout\":%s,"
		"\"hidden_chase_timeouts_total\":%d,"
		"\"hidden_wall_timeout\":%s,"
		"\"hidden_wall_timeouts_total\":%d},"
		...
		qge_noesis_assist_hidden_chase_timeout ? "true" : "false",
		qge_noesis_assist_hidden_chase_timeouts_total,
		qge_noesis_assist_hidden_wall_timeout ? "true" : "false",
		qge_noesis_assist_hidden_wall_timeouts_total,
...
void QGE_FrameEnd(void)
{
	...
	if (quantum_debug.value >= 1.0f && qge_scene_surface_count) {
		fprintf(stderr, "QGE scene frame=%d world_surfaces=%d submitted=%d dropped=%d "
				...
				"light_entries=%d poly=%d gaprepair=%d texfilter=%d culled=%d surrogate=%d micro=%d clipped=%d invalid=%d "
...
void QGE_SceneBegin(void)
{
	...
	qge_scene_world_gap_repairs = 0;
	qge_scene_texture_prefilter_samples = 0;
	...
	float t)
{
	...
	inv_depth = ia + (ib - ia) * t;
	if (inv_depth > 0.000001f) {
		float depth = 1.0f / inv_depth;
		out.depth = depth;
		out.tex_s = ((a->tex_s * ia) +
					 (b->tex_s * ib - a->tex_s * ia) * t) * depth;
		out.tex_t = ((a->tex_t * ia) +
					 (b->tex_t * ib - a->tex_t * ia) * t) * depth;
		out.light_s = ((a->light_s * ia) +
					   (b->light_s * ib - a->light_s * ia) * t) * depth;
		out.light_t = ((a->light_t * ia) +
					   (b->light_t * ib - a->light_t * ia) * t) * depth;
	} else {
...
QGE_HOT_INLINE qboolean QGE_SpatialDepthHasWorld(float depth)
{
	return depth > 0.0f &&
		isfinite(depth) &&
		depth < QGE_SPATIAL_DEPTH_FAR * QGE_SPATIAL_SEAM_REPAIR_DEPTH_RATIO;
}

static void QGE_SpatialRepairWorldCoverageGaps(void)
{
	float *depth = qge_spatial_depth_buffer;
	float *rbuf = qge_spatial_color_buffer[QGE_DWT_R];
	float *gbuf = qge_spatial_color_buffer[QGE_DWT_G];
	float *bbuf = qge_spatial_color_buffer[QGE_DWT_B];
	int repairs = 0;

	if (!depth || !rbuf || !gbuf || !bbuf || qge_render_res < 3)
		return;

	for (int y = 1; y + 1 < qge_render_res; y++) {
		for (int x = 1; x + 1 < qge_render_res; x++) {
			int idx = y * qge_render_res + x;
			float r = 0.0f;
			float g = 0.0f;
			float b = 0.0f;
			int neighbors = 0;

			if (QGE_SpatialDepthHasWorld(depth[idx]))
				continue;
			...
			for (int dy = -1; dy <= 1; dy++) {
				for (int dx = -1; dx <= 1; dx++) {
					...
					if (dx == 0 && dy == 0)
						continue;
					nidx = (y + dy) * qge_render_res + (x + dx);
					if (!QGE_SpatialDepthHasWorld(depth[nidx]))
						continue;
					r += rbuf[nidx];
					g += gbuf[nidx];
					b += bbuf[nidx];
			...
			if (neighbors < QGE_SPATIAL_SEAM_REPAIR_MIN_NEIGHBORS)
				continue;
			rbuf[idx] = r / (float)neighbors;
			gbuf[idx] = g / (float)neighbors;
			bbuf[idx] = b / (float)neighbors;
			/*
			 * Keep the far depth so repaired coverage cannot seed a wider
			 * flood fill and cannot occlude later entity/particle passes.
			 */
			repairs++;
		}
	}
	qge_scene_world_gap_repairs = repairs;
}
																...
																int num_verts)
{
	...
	sample.tex_footprint /= (float)num_verts;
	...
	qge_projected_triangle_sampler_t *sampler)
{
	...
	sampler->inv_depth_dy =
		w0_y * (sampler->ia - sampler->ic) +
		w1_y * (sampler->ib - sampler->ic);
	sampler->tex_s_num_dx =
		w0_x * (sampler->a_tex_s_ia - sampler->c_tex_s_ic) +
		w1_x * (sampler->b_tex_s_ib - sampler->c_tex_s_ic);
	sampler->tex_s_num_dy =
		w0_y * (sampler->a_tex_s_ia - sampler->c_tex_s_ic) +
		w1_y * (sampler->b_tex_s_ib - sampler->c_tex_s_ic);
	sampler->tex_t_num_dx =
		w0_x * (sampler->a_tex_t_ia - sampler->c_tex_t_ic) +
		w1_x * (sampler->b_tex_t_ib - sampler->c_tex_t_ic);
	sampler->tex_t_num_dy =
		w0_y * (sampler->a_tex_t_ia - sampler->c_tex_t_ic) +
		w1_y * (sampler->b_tex_t_ib - sampler->c_tex_t_ic);
	sampler->light_s_num_dx =
		w0_x * (sampler->a_light_s_ia - sampler->c_light_s_ic) +
		w1_x * (sampler->b_light_s_ib - sampler->c_light_s_ic);
	sampler->light_s_num_dy =
		w0_y * (sampler->a_light_s_ia - sampler->c_light_s_ic) +
		w1_y * (sampler->b_light_s_ib - sampler->c_light_s_ic);
	sampler->light_t_num_dx =
		w0_x * (sampler->a_light_t_ia - sampler->c_light_t_ic) +
		w1_x * (sampler->b_light_t_ib - sampler->c_light_t_ic);
	sampler->light_t_num_dy =
		w0_y * (sampler->a_light_t_ia - sampler->c_light_t_ic) +
		w1_y * (sampler->b_light_t_ib - sampler->c_light_t_ic);
	...
	float *coverage)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		sample->tex_footprint += sub.tex_footprint;
	...
	sample->tex_footprint /= (float)hits;
...
static float QGE_ProjectedTriangleTextureFootprint(
	const qge_projected_triangle_sampler_t *sampler,
	...
	float tex_t_num)
{
	...
	if (!sampler || !sampler->valid || !ctx ||
		!ctx->tex_width || !ctx->tex_height ||
		!(inv_depth > 0.000001f))
		return 1.0f;

	tex_s = tex_s_num / inv_depth;
	tex_t = tex_t_num / inv_depth;

	if (inv_depth + sampler->inv_depth_dx > 0.000001f) {
		float sx = (tex_s_num + sampler->tex_s_num_dx) /
				   (inv_depth + sampler->inv_depth_dx);
		float tx = (tex_t_num + sampler->tex_t_num_dx) /
				   (inv_depth + sampler->inv_depth_dx);
		float ds = (sx - tex_s) * ctx->tex_width_f;
		float dt = (tx - tex_t) * ctx->tex_height_f;
		float len = sqrtf(ds * ds + dt * dt);
		if (len > footprint)
			footprint = len;
	}
	if (inv_depth + sampler->inv_depth_dy > 0.000001f) {
		float sy = (tex_s_num + sampler->tex_s_num_dy) /
				   (inv_depth + sampler->inv_depth_dy);
		float ty = (tex_t_num + sampler->tex_t_num_dy) /
				   (inv_depth + sampler->inv_depth_dy);
		float ds = (sy - tex_s) * ctx->tex_width_f;
		float dt = (ty - tex_t) * ctx->tex_height_f;
		float len = sqrtf(ds * ds + dt * dt);
		if (len > footprint)
			footprint = len;
	}

	if (!isfinite(footprint) || footprint < 1.0f)
		return 1.0f;
	if (footprint > QGE_TEXTURE_PREFILTER_MAX_FOOTPRINT)
		return QGE_TEXTURE_PREFILTER_MAX_FOOTPRINT;
	return footprint;
}

static float QGE_ProjectedTriangleTextureFootprintAt(
	const qge_projected_triangle_sampler_t *sampler,
	const qge_surface_sample_context_t *ctx,
	float x,
	float y)
{
	...
	w2 = 1.0f - w0 - w1;
	inv_depth = w0 * sampler->ia + w1 * sampler->ib + w2 * sampler->ic;
	tex_s_num = w0 * sampler->a_tex_s_ia +
				w1 * sampler->b_tex_s_ib +
				w2 * sampler->c_tex_s_ic;
	tex_t_num = w0 * sampler->a_tex_t_ia +
				w1 * sampler->b_tex_t_ib +
				w2 * sampler->c_tex_t_ic;
	return QGE_ProjectedTriangleTextureFootprint(sampler, ctx, inv_depth,
												 tex_s_num, tex_t_num);
}
	...
	const qge_projected_sample_t *sample)
{
	...
	surf = ctx->surf;
	if (ctx->bilinear) {
		float sf = sample->light_s * ctx->light_s_scale +
				   ctx->light_s_offset - 0.5f;
		float tf = sample->light_t * ctx->light_t_scale +
				   ctx->light_t_offset - 0.5f;
		int s1, t1;
		float s_frac, t_frac;
		...
		if (s1 >= ctx->light_smax) s1 = ctx->light_smax - 1;
		if (t1 >= ctx->light_tmax) t1 = ctx->light_tmax - 1;
		w00 = (1.0f - s_frac) * (1.0f - t_frac);
		w10 = s_frac * (1.0f - t_frac);
		w01 = (1.0f - s_frac) * t_frac;
		w11 = s_frac * t_frac;
		...
		for (int map = 0; map < ctx->light_map_count; map++) {
			const byte *base = surf->samples + map * ctx->light_size * 3;
			const byte *p00 = base + (t0 * ctx->light_smax + s0) * 3;
			const byte *p10 = base + (t0 * ctx->light_smax + s1) * 3;
			const byte *p01 = base + (t1 * ctx->light_smax + s0) * 3;
			const byte *p11 = base + (t1 * ctx->light_smax + s1) * 3;
			float scale = ctx->light_scales[map];

			color.r += ((float)p00[0] * w00 + (float)p10[0] * w10 +
						(float)p01[0] * w01 + (float)p11[0] * w11) * scale;
			color.g += ((float)p00[1] * w00 + (float)p10[1] * w10 +
						(float)p01[1] * w01 + (float)p11[1] * w11) * scale;
			color.b += ((float)p00[2] * w00 + (float)p10[2] * w10 +
						(float)p01[2] * w01 + (float)p11[2] * w11) * scale;
		}
		if (color.r > 1.35f) color.r = 1.35f;
		if (color.g > 1.35f) color.g = 1.35f;
		if (color.b > 1.35f) color.b = 1.35f;
...
static int QGE_SurfaceTextureWrappedCoord(int value, unsigned int size,
										  unsigned int mask,
										  qboolean power2)
{
	if (size == 0)
	...
	float *fullbright_b)
{
	...
	x = QGE_SurfaceTextureWrappedCoord(x, ctx->tex_width,
									   ctx->tex_width_mask,
									   ctx->tex_power2);
	y = QGE_SurfaceTextureWrappedCoord(y, ctx->tex_height,
									   ctx->tex_height_mask,
									   ctx->tex_power2);
	...
	qge_rgb_sample_t *fullbright)
{
	...
	if (tex_footprint > QGE_TEXTURE_PREFILTER_MAX_FOOTPRINT)
		tex_footprint = QGE_TEXTURE_PREFILTER_MAX_FOOTPRINT;
	...
	qge_rgb_sample_t *fullbright)
{
	...
	if (ctx->warp) {
		/*
		 * Quake stores the first turbulent world poly in already-scaled
		 * coordinates, while subdivided warp polys use raw texture vectors.
		 * Sample both forms through roughly the same 64-unit repeating space.
		 */
		if (fabsf(tex_s) < 64.0f && fabsf(tex_t) < 64.0f) {
			tex_s *= 2.0f;
			tex_t *= 2.0f;
		} else {
			tex_s *= 1.0f / 64.0f;
	...
	if (ctx->bilinear && tex_footprint >= QGE_TEXTURE_BILINEAR_MIN_FOOTPRINT) {
		...
		fx = tex_s * ctx->tex_width_f - 0.5f;
		fy = tex_t * ctx->tex_height_f - 0.5f;
		x0 = (int)floorf(fx);
		y0 = (int)floorf(fy);
		x1 = x0 + 1;
		y1 = y0 + 1;
		...
		if (ctx->tex_power2) {
			x0 &= (int)ctx->tex_width_mask;
			x1 &= (int)ctx->tex_width_mask;
			y0 &= (int)ctx->tex_height_mask;
			y1 &= (int)ctx->tex_height_mask;
		} else {
			x0 %= (int)ctx->tex_width;
			x1 %= (int)ctx->tex_width;
			y0 %= (int)ctx->tex_height;
			y1 %= (int)ctx->tex_height;
			if (x0 < 0) x0 += (int)ctx->tex_width;
			if (x1 < 0) x1 += (int)ctx->tex_width;
			if (y0 < 0) y0 += (int)ctx->tex_height;
			if (y1 < 0) y1 += (int)ctx->tex_height;
		}

		p00 = ctx->tex_pixels[y0 * (int)ctx->tex_width + x0];
				...
				for (int x = sx1, x_offset = 0; x <= sx2;
					...
					if (!(inv_depth > 0.000001f)) {
						...
						if (!QGE_ProjectedTriangleSampleWeightsUnchecked(&sampler,
																		 ...
																		 &sample))
							continue;
						sample.tex_footprint =
							QGE_ProjectedTriangleTextureFootprintAt(
								&sampler, &sample_ctx,
								(float)x + 0.5f, sample_y);
					} else {
						...
						sample.tex_footprint =
							QGE_ProjectedTriangleTextureFootprint(
								&sampler, &sample_ctx, inv_depth,
								tex_s_num, tex_t_num);
...
static void QGE_EncodeScene(void)
{
	...
	QGE_SpatialRepairWorldCoverageGaps();
