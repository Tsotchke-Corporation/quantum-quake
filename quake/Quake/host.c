...
// decide the simulation time
	...
	QGE_FrameBegin ();  /* Start quantum frame timing */
...
// update audio
	...
	QGE_FrameEnd ();  /* End quantum frame timing */
...
void Host_Init (void)
{
	...
	if (cls.state != ca_dedicated)
	{
		...
		R_Init ();
		S_Init ();
		QGE_Init ();  /* Quantum Game Engine — after audio, before CDAudio */
		CDAudio_Init ();
		BGM_Init();
		Sbar_Init ();
		CL_Init ();
...
// keep Con_Printf from trying to update the screen
	...
	if (cls.state != ca_dedicated)
	{
		...
		BGM_Shutdown();
		CDAudio_Shutdown ();
		QGE_Shutdown ();  /* Quantum Game Engine — before audio shutdown */
		S_Shutdown ();
		IN_Shutdown ();
		VID_Shutdown();
	}
