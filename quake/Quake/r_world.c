...
static void R_BindFullbrightTextureOrNull(gltexture_t *fullbright)
{
...
void R_DrawTextureChains_Water (qmodel_t *model, entity_t *ent, texchain_t chain)
{
	...
	else if (cl.worldmodel->haslitwater && r_litwater.value && r_world_program != 0)
	{
		...
		R_BindFullbrightTextureOrNull (NULL);
...
// set uniforms
	...
	// Enable/disable TMU 2 (fullbrights)
		...
		{
			R_BindFullbrightTextureOrNull (fullbright);
...
void R_DrawWorld (void)
{
	...
	/* Setup quantum visibility viewpoint for Grover-accelerated BSP culling */
	if (quantum_vis.value >= 0.5f)
	{
		QGE_VisSetupViewpoint(
