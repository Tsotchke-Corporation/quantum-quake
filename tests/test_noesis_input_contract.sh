...
grep -Fq 'noesis_min_capture_wait="${QGE_NOESIS_MIN_CAPTURE_WAIT:-192}"' "$repo_root/tools/quake_graphics_stream.sh"
...
grep -Fq 'noesis_assist="${QGE_NOESIS_ASSIST:-1}"' "$repo_root/tools/quake_graphics_stream.sh"
grep -Fq 'noesis_assist="$(normalize_nonnegative_int "$noesis_assist" 1)"' "$repo_root/tools/quake_graphics_stream.sh"
...
grep -Fq 'move->sidemove = left_clear >= right_clear ? -320.0f : 320.0f;' "$repo_root/quake/Quake/qge_hooks.c"
