...
grep -q 'samples prepared surface texture and' "$repo_root/docs/qge_agent_stream.md"
...
grep -q 'QGE_TEXTURE_BILINEAR_MIN_FOOTPRINT 1.15f' "$repo_root/quake/Quake/qge_hooks.c"
...
grep -q 'magnified and one-texel footprint' "$repo_root/docs/qge_agent_stream.md"
...
grep -q 'Latest verified runtime baseline is the current QGE moderate-minification' "$repo_root/docs/qge_state_of_development.md"
grep -q 'diagnostic notify cleanup' "$repo_root/README.md"
grep -q 'diagnostic notify cleanup' "$repo_root/docs/qge_state_of_development.md"
grep -q 'world fullbright sampling scale, and' "$repo_root/README.md"
