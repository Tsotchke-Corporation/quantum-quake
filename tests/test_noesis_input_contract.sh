...
grep -q 'QGE_TEXTURE_PREFILTER_MIN_FOOTPRINT 2.25f' "$repo_root/quake/Quake/qge_hooks.c"
...
grep -q 'Mild minification uses the nine-tap path' "$repo_root/docs/qge_agent_stream.md"
...
grep -q 'Latest verified runtime baseline is the current QGE alias-skin viewmodel' "$repo_root/docs/qge_state_of_development.md"
...
grep -q 'world fullbright sampling scale, and diagnostic notify cleanup' "$repo_root/README.md"
